...
foreach(install_path ${ROS2_INSTALL_PATHS})
  ...
  foreach(distro ${ROS2_DISTROS})
    ...
    if(EXISTS "${ros2_include_path}")
      ...
      # Some ROS2 packages use nested include layout, e.g. rclcpp/rclcpp.hpp
      # located at <prefix>/include/rclcpp/rclcpp/rclcpp.hpp.
      # Add only package roots that mirror this pattern and explicitly avoid
      # the plain "dds" include path, which can shadow libc's <features.h>.
      file(GLOB _ROS2_PKG_DIRS "${ros2_include_path}/*")
      foreach(pkg_dir ${_ROS2_PKG_DIRS})
        if(IS_DIRECTORY "${pkg_dir}")
          get_filename_component(pkg_name "${pkg_dir}" NAME)
          if(NOT pkg_name STREQUAL "dds" AND EXISTS "${pkg_dir}/${pkg_name}")
            list(APPEND ROS2_INCLUDE_DIRS "${pkg_dir}")
          endif()
        endif()
      endforeach()
...
option(BUILD_ROS2_TESTS "Build ROS2 test executable (test_ros2)" OFF)
if(BUILD_ROS2_TESTS)
  ...
  message(STATUS "ROS2 test executable disabled (BUILD_ROS2_TESTS=OFF)")
endif()
