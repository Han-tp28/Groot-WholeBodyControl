...
class G1Deploy {
  ...
  public:
    ...
    G1Deploy(
      ...
      const bool is_simulation_mode = disable_crc_check_;
      std::cout << "[DEBUG] [Init] Simulation mode: " << (is_simulation_mode ? "true" : "false") << std::endl;
      ...
      try {
        ...
        const bool loopback_requested =
          (networkInterface == "lo" || networkInterface == "lo0" || networkInterface == "127.0.0.1");
        if (is_simulation_mode && loopback_requested) {
          std::cerr << "[WARN] DDS init on loopback interface '" << networkInterface
                    << "' failed, retrying with default interface selection..." << std::endl;
          try {
            ChannelFactory::Instance()->Init(0);
            std::cerr << "[INFO] DDS init retry succeeded using default interface selection." << std::endl;
          } catch (const std::exception& retry_e) {
            throw std::runtime_error(
              "Failed to initialize DDS ChannelFactory on loopback '" + networkInterface +
              "', and fallback init failed: " + std::string(retry_e.what()));
          } catch (...) {
            throw std::runtime_error(
              "Failed to initialize DDS ChannelFactory on loopback '" + networkInterface +
              "', and fallback init failed with an unknown error.");
          }
        } else {
        throw std::runtime_error(
          "Failed to initialize DDS ChannelFactory on interface '" + networkInterface +
          "': " + std::string(e.what()));
        }
      } catch (...) {
        throw std::runtime_error(
      ...
      std::cout << "[DEBUG] [Init] DDS ChannelFactory initialized" << std::endl;

      // Initialize Dex3 hands only for real robot runs.
      // In simulation this can interact with unavailable hardware channels.
      if (!is_simulation_mode) {
        std::cout << "[DEBUG] [Init] Initializing Dex3 hands" << std::endl;
        dex3_hands_.initialize("");
      } else {
        std::cout << "[DEBUG] [Init] Skipping Dex3 hands initialization (sim mode)" << std::endl;
      }

      if (!is_simulation_mode) {
        std::cout << "[DEBUG] [Init] Initializing AudioThread" << std::endl;
        audio_thread_ = std::make_unique<AudioThread>();
      } else {
        std::cout << "[DEBUG] [Init] Skipping AudioThread initialization (sim mode)" << std::endl;
      }
      ...
      // Try to shutdown motion control-related service only on real robot runs.
      // In simulation mode this service is often unavailable and can destabilize init.
      if (!is_simulation_mode) {
        ...
        std::cout << "[INFO] Simulation mode detected - skipping MotionSwitcherClient initialization" << std::endl;
      }
...
int main(int argc, char const* argv[]) {
  ...
  std::unique_ptr<G1Deploy> custom;
  try {
    custom = std::make_unique<G1Deploy>(
      networkInterface,
      modelFile,
      motionDataPath,
      disableCrcCheck,
    ...
    std::cerr << "[FATAL] Failed to initialize G1Deploy: " << e.what() << std::endl;
    return 1;
  } catch (...) {
    std::cerr << "[FATAL] Failed to initialize G1Deploy due to an unknown exception." << std::endl;
    return 1;
  }
